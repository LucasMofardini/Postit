...
-- Copiando dados para a tabela postit.artigo: ~9 rows (aproximadamente)
...
INSERT IGNORE INTO `artigo` (`id`, `titulo`, `conteudo`, `horario_publicacao`) VALUES
	...
	(23, 'Teste Clone', 'Este artigo foi clonado!!', '2021-04-24 22:38:58'),
	(24, 'Teste Clone', 'Este artigo foi clonado!!', '2021-04-24 22:39:32');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_adicionaArtigo`(
    ...
    COMMENT 'Procedure que adiciona artigo, onde os parametros devem ser o Titulo e o Conteudo a ser adicionado'
BEGIN
...
-- Copiando estrutura para procedure postit.sp_clonaArtigo
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_clonaArtigo`(
	IN `pIdArtigo` INT

)
    COMMENT 'Este parametro clona um artigo e recebe como parametro o ID do artigo a ser clonado'
BEGIN
	
	select @varTitulo := titulo,
	  		 @varConteudo := conteudo
	  from artigo
	 where id = pIdArtigo;
	 
	 insert into artigo(titulo, conteudo, horario_publicacao) values(@varTitulo, @varConteudo, current_timestamp());
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_editaArtigo`(
    ...
    COMMENT 'Procedure que edita artigo, onde os parametros devem ser o Id, Titulo e o Conteudo a ser editado'
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_removeArtigo`(
    ...
    COMMENT 'Procedure que remove artigo, onde o parametro devem ser o ID do a ser removido '
BEGIN
